...
export default async function AccountsPage() {
  ...
  return (
    <div className="accounts-main mx-auto w-full max-w-[1276px]">
      ...
      <div className="mt-3 grid gap-3 lg:grid-cols-[minmax(0,1.45fr)_minmax(320px,.75fr)]"><Card className="min-h-[258px] !p-4 sm:!p-5" title="Recent contributions"><div className="flex justify-end"><AccountManagerLink className="-mt-8 text-xs font-semibold text-blue-600">View all</AccountManagerLink></div><table className="w-full text-left text-xs"><thead className="text-[var(--text-muted)]"><tr><th className="py-2 font-medium">Date</th><th className="py-2 font-medium">Account</th><th className="py-2 font-medium">Amount</th><th className="py-2 font-medium">Type</th><th className="py-2 font-medium">Status</th></tr></thead><tbody>{contributions.slice(-5).reverse().map(c=><tr key={c.id} className="border-t border-[var(--border)]"><td className="py-2">{new Date(`${c.contributed_on}T12:00:00`).toLocaleDateString('en-CA')}</td><td>{c.account}</td><td className="font-semibold">{formatCAD(c.amount_cad)}</td><td>Contribution</td><td className="text-emerald-600">● Completed</td></tr>)}{!contributions.length&&<tr><td colSpan={5} className="py-12 text-center text-sm text-[var(--text-muted)]">No contributions yet.</td></tr>}</tbody></table></Card><Card className="min-h-[258px] !p-4 sm:!p-5" title="Account types"><div className="space-y-2">{[['◉','Tax-Free Savings Account (TFSA)','Grow your investments tax-free.','bg-emerald-50 text-emerald-600'],['▥','Registered Retirement Savings Plan (RRSP)','Save for retirement with tax advantages.','bg-blue-50 text-blue-600'],['▣','Cash account','Keep cash for short-term needs.','bg-orange-50 text-orange-500']].map(([icon,title,copy,tone])=><div key={title} className="flex min-h-[58px] items-center gap-3 rounded-lg border border-[var(--border)] px-2 py-1.5"><span className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${tone}`}>{icon}</span><span className="min-w-0 flex-1"><b className="block text-xs">{title}</b><small className="block text-[11px] leading-4 text-[var(--text-muted)]">{copy}</small></span></div>)}</div></Card></div>
    </div>
  );
}
