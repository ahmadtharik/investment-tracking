  ...
  return (
    <div>
      ...
      <div className="relative mt-2 h-48">
        {visible.length > 0 && <ResponsiveContainer className="relative z-10" width="100%" height="100%"><PieChart><Pie data={visible} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius="62%" outerRadius="88%" paddingAngle={visible.length > 1 ? 2 : 0} stroke="none" isAnimationActive={false}>{visible.map((d, i) => <Cell key={d.key} fill={colors[i % colors.length]} />)}</Pie><Tooltip wrapperStyle={{ zIndex: 50 }} contentStyle={{ backgroundColor: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 10, boxShadow: '0 8px 24px rgba(53,48,36,0.16)', color: 'var(--ink)' }} formatter={(v) => formatCAD(Number(v))} /></PieChart></ResponsiveContainer>}
        <div className="pointer-events-none absolute inset-0 z-0 flex flex-col items-center justify-center"><span className="text-xs text-zinc-500">Total</span><span className="text-lg font-semibold tabular-nums">{formatCAD(total)}</span></div>
      </div>
